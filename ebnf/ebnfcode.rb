...
class EbnfCode
  ...
  def flatten_rht(input)
    ...
    if input.is_a?(Lexeme)
      ...
      when :subterm # flatten subterm to name or array then flatten again
        to_return = if input.children.first.type == :name
      ...
      input.each do |term|
        ...
        to_return << if flattened_term.length == 1
                       flattened_term.first
                     else
                       flattened_term
                     end
      end
  ...
  def rhts_to_s_array(rhts_pointer)
    ...
    reformat(flatten_rht(rhts_pointer)).collect do |primitive_rht_term_array|
  ...
  def rule_to_s(rule)
    ...
    rhts = rhts_to_s_array(rule.children[2]).uniq
    ...
    primitive_strings = rhts.collect do |rht_string|
      if skip_next
        skip_next = false
        "#{repeat_lht}=#{rht_string.split(',').reverse[1..].join(',')}".gsub(/__repeat__,.*?,/, '')
      elsif rht_string.split(',').last == '__repeat__'
        skip_next = true
